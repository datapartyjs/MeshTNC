...
int main () {
  ...
  try {
    ...
    if (!serial.is_open()) {
      printf("FATAL: Failed to open serial port '%s'", SERIAL_PORT);
      std::exit(1);
    }
    ...
    printf("FATAL: Failed to initialize serial port '%s': %s", SERIAL_PORT, e.what());
    std::exit(1);
  }
  ...
  if (serial.is_open()) {
    ...
    while (true) {
      try {
        ...
        printf("error reading serial port: %s", ec.message());
      }
      printf("%s {%i}\r\n", rd_buf.data(), rd_len);
    }
    serial.close();
